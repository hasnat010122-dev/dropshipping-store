...
export async function sendOrderConfirmationEmail(order: OrderRow) {
  ...
  return deliver({
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto;color:#14141c"><h2>Thanks, ${escapeHtml(order.customerName)}!</h2><p>We received your order. It is awaiting owner review before supplier processing.</p><table style="width:100%;border-collapse:collapse;margin:20px 0">${itemsHtml}<tr style="border-top:1px solid #ddd;font-weight:700"><td style="padding:10px 0">Total</td><td style="padding:10px 0;text-align:right">${formatUSD(order.total)}</td></tr></table><p style="font-size:14px">Payment method: ${escapeHtml(paymentLabel(order.paymentMethod))}<br>Delivery: ${escapeHtml(order.address)}, ${escapeHtml(order.city)}</p>${order.paymentMethod === BANK_TRANSFER.id ? `<div style="border:1px solid #ddd;padding:16px;margin-top:20px"><strong>Bank transfer details</strong><p>Account number: ${BANK_TRANSFER.accountNumber}</p><p>Transfer exactly Rs ${order.total.toLocaleString("en-PK")} and use ${order.id.slice(0, 8).toUpperCase()} as the reference. Keep your receipt until payment is confirmed.</p></div>` : ""}<p style="color:#777;font-size:12px;margin-top:30px">Order ID: ${escapeHtml(order.id)}</p></div>`,
  });
}
