...
function productFrom(row: Record<string, any>): ProductRow {
  return { id: row.id, name: row.name, price: Number(row.price), compareAt: row.compare_at == null ? null : Number(row.compare_at), category: row.category, badge: row.badge, image: row.image, images: Array.isArray(row.images) && row.images.length ? row.images : [row.image], colors: Array.isArray(row.colors) ? row.colors : [], colorImages: row.color_images && typeof row.color_images === "object" ? row.color_images : {}, description: row.description, stock: Number(row.stock), supplierId: row.supplier_id, supplierProductUrl: row.supplier_product_url, supplierCost: row.supplier_cost == null ? null : Number(row.supplier_cost), publicationStatus: row.publication_status, createdAt: row.created_at };
}
function productTo(row: ProductRow) {
  return { id: row.id, name: row.name, price: row.price, compare_at: row.compareAt, category: row.category, badge: row.badge, image: row.image, images: row.images || [row.image], colors: row.colors || [], color_images: row.colorImages || {}, description: row.description, stock: row.stock, supplier_id: row.supplierId, supplier_product_url: row.supplierProductUrl, supplier_cost: row.supplierCost, publication_status: row.publicationStatus || "published", created_at: row.createdAt };
}
