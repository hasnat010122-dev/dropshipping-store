...
export default function AddToCartBox({
  ...
  const needsColor = Boolean(product.colors?.length && !selectedColor);
  ...
  return (
    <div className="flex gap-3 mb-8">
      <motion.button
        onClick={handleAdd}
        disabled={outOfStock || needsColor}
        whileHover={!outOfStock && !needsColor ? { scale: 1.02 } : {}}
        whileTap={!outOfStock && !needsColor ? { scale: 0.97 } : {}}
      ...
      >
        <AnimatePresence mode="wait">
          <motion.span
            key={outOfStock ? "out" : needsColor ? "color" : added ? "added" : "add"}
            ...
            className="block"
          >
            {outOfStock ? "Out of stock" : needsColor ? "Select a color" : added ? "Added ✓" : "Add to cart"}
          </motion.span>
        </AnimatePresence>
      </motion.button>
      <motion.button
